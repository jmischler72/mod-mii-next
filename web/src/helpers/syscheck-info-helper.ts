...
export function getConsoleRegion(data: string): string | null {
	...
	const region = regionLine.split(':')[1].trim().toUpperCase();

	// Return the full region name instead of the short code
	if (region === 'PAL') return 'PAL';
	if (region === 'NTSC-U') return 'NTSC-U';
	if (region === 'NTSC-J' || region === 'JAP') return 'NTSC-J';
	if (region === 'KOR') return 'KOR';

	return null;
}

export function getRegionShortCode(region: string): string {
	switch (region) {
		case 'PAL':
			return 'E';
		case 'NTSC-U':
...
export function getSystemMenuVersion(data: string): string | null {
	...
	const version = systemMenuLine
		.replace(/^.*System Menu\s*/, '')
		.replace(/^\s*|\s*$/g, '')
		.replace(/,/g, '');
	return version || null;
}
...
): { firmware: string; firmwareVersion: number; SMregion: string } | null {
...
export function getLatestSMVersion(data: { firmware: string; firmwareVersion: number }): string {
	...
	if (data.firmwareVersion > 518) {
		// check if firmware version is a cSM (custom System Menu)
